...
def root():
    x = 5.4 #x0 # iteration'ı bunla başlatıyoruz, o sonra buluyor.
    iters = 0
    print(x)
    while(True): # do while loop yapıldı aslında
        fx = x**2 - 11*x + 28  # bu fonk kökleri 4 ve 7 'de
        
        xx, y, z = symbols('x y z')
        init_printing(use_unicode=True)
        gx = diff( xx**2 - 11*xx + 28 , xx)
        gx = lambdify(xx, gx, "numpy") # böyle numpy da yazılarak bu işlemi yapaken numpy'ın mı kullanılması sağlanıyormuş öyle bir şey dedi hoca.
        gx = gx(x) #gx bi func gibi oldu, lambda func falan alakalıymış bu durum. gx sembolikti, gx'in sembolik karakteri xx içinde bir değer olarak x kondu hesap yaptı.
...
# %% q7 # daha hızlı yapılabilir. PDF'te soruda ne denmek istediği daha iyi açıklandı, ona göre de kodlanabilir.
# hocanın çözümü daha iyi. Kısa ve hızlı
factorize_this = 163352 # 163352 #16335689prime uzun sürüyor # 163351prime #16335689  #3757208
